...
//                add(l1);
		...
		b1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
                            try {
            String s1 = c1.getSelectedItem(); // Get selected customer ID
            conn c = new conn();  // Create connection

            // First Query: Get Room Number from Customer Table
            ResultSet rs1 = c.s.executeQuery("SELECT * FROM customer WHERE number = '" + s1 + "'");
            if (rs1.next()) {
                txt_Room.setText(rs1.getString("room"));  // Update room number field
            } else {
                JOptionPane.showMessageDialog(null, "No customer found with ID: " + s1);
                return;
            }

            if (!txt_Room.getText().isEmpty()) {            // Second Query: Get Room Availability & Cleaning Status
                ...
                if (rs2.next()) {
                    ...
                    txt_Status.setText(rs2.getString("cleaing_status")); // Corrected column name
                } else {
                    JOptionPane.showMessageDialog(null, "No room found for Room Number: " + txt_Room.getText());
                }
            }
        } catch (Exception ee) {
            ee.printStackTrace();
